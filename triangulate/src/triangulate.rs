...
fn advanced_face(
    ...
    let face = s
        .entity(f)
        .ok_or(Error::InvalidStepEntity("AdvancedFace"))?;
    ...
    stats.num_faces += 1;
    info!(
        "triangulating face {} (geometry {})",
        f.0, face.face_geometry.0
    );

    // Grab the surface, returning early if it's unimplemented
    let mut surf = get_surface(s, face.face_geometry)?;
    if torus_annulus_face(s, face) {
        surf.prefer_torus_annulus_lowering();
    }
    ...
    let result = {
        ...
        loop {
            let mut t = match cdt::Triangulation::new_with_edges(&pts, &edges) {
                Err(e) if !retried_without_steiner && n_steiner > 0 => {
                    info!(
                        "face {}: {:?} while building CDT, retrying without {} steiner points \
                         ({} pts, {} edges)",
                        face_id,
                        e,
                        n_steiner,
            ...
            match t.run() {
                ...
                Err(e) if !retried_without_steiner && n_steiner > 0 => {
                    info!(
                        "face {}: {:?}, retrying without {} steiner points ({} pts, {} edges)",
                        face_id,
                        e,
                        n_steiner,
                        pts.len(),
    ...
    match result {
        Ok(t) => {
            let triangles: Vec<_> = t.triangles().collect();
            let face_same_sense = face.same_sense ^ surf.winding_flip();
            for (a, b, c) in triangles {
                let a = (a + offset) as u32;
                let b = (b + offset) as u32;
                let c = (c + offset) as u32;
                mesh.triangles.push(Triangle {
                    verts: if face_same_sense {
                        U32Vec3::new(a, b, c)
                    } else {
                        U32Vec3::new(a, c, b)
                    },
    ...
    info!(
        "face {} post-cdt: applying colors/normals ({} verts from v_start)",
        face_id,
        mesh.verts.len() - v_start
    );
...
fn torus_annulus_face(s: &StepFile, face: &AdvancedFace_) -> bool {
    if !matches!(&s[face.face_geometry], Entity::ToroidalSurface(_)) {
        return false;
    }
    face.bounds
        .iter()
        .all(|bound| closed_circle_bound(s, (*bound).cast()))
}

fn closed_circle_bound(s: &StepFile, bound: FaceBound) -> bool {
    let bound_loop = match &s[bound] {
        Entity::FaceBound(b) => b.bound,
        Entity::FaceOuterBound(b) => b.bound,
        _ => return false,
    };
    let Entity::EdgeLoop(loop_) = &s[bound_loop] else {
        return false;
    };
    if loop_.edge_list.len() != 1 {
        return false;
    }
    let Some(oriented) = s.entity(loop_.edge_list[0]) else {
        return false;
    };
    let Some(edge_curve) = s.entity(oriented.edge_element.cast::<EdgeCurve_>()) else {
        return false;
    };
    edge_curve.edge_start == edge_curve.edge_end && curve_is_circle(s, edge_curve.edge_geometry)
}

fn curve_is_circle(s: &StepFile, curve_id: ap214::Curve) -> bool {
    match &s[curve_id] {
        Entity::Circle(_) => true,
        Entity::SurfaceCurve(curve) => curve_is_circle(s, curve.curve_3d),
        Entity::SeamCurve(curve) => curve_is_circle(s, curve.curve_3d),
        _ => false,
    }
}
