    ...
    for path in files {
        ...
        match result {
            ...
            None => {
                ...
                results.push(FileResult {
                    name,
                    duration_ms: PER_FILE_TIMEOUT.as_secs_f64() * 1000.0,
                    triangles: 0,
                    faces: 0,
                    errors: 0,
                    panics: 1,
...
fn main() {
    let matches = App::new("regression_test")
        ...
        .arg(
            Arg::with_name("timing-threshold")
    ...
    log::set_boxed_logger(Box::new(CountingLogger { inner })).expect("Failed to set logger");
    ...
    if files.is_empty() {
        eprintln!(
            "No STEP files found in {}/ — set TEST_ASSETS_DIR to override.",
            assets_dir
        );
        std::process::exit(1);
    }
    ...
    if let Some(path) = matches.value_of("compare") {
        let threshold: f64 = matches
            .value_of("timing-threshold")
            .unwrap()
            .parse()
            .expect("Invalid timing threshold");
        ...
        for r in &results {
            if let Some(&(base_time, base_err, base_panic, base_warn, base_log_err)) =
                baseline.get(&r.name)
            {
